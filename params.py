...
def get_params():
    ...
    args.add_argument("-step", "--step", default="train", type=str, choices=['train', 'test', 'dev', 'finetune'])
    ...
    args.add_argument("-finetune", "--finetune", default=False, type=bool)
    args.add_argument("-finetune_ckpt", "--finetune_ckpt", default=None, type=str)
...
data_dir = {
    'train_tasks': '/train_tasks.json', # already orgainzed according to relation
    'test_tasks': '/test_tasks.json',
    'dev_tasks': '/dev_tasks.json',
    'rel2candidates': '/rel2candidates.json', # candidate find by another method 
    'e1rel_e2': '/e1rel_e2.json', # er:[e2,e2,e2...]
    'ent2ids': '/ent2ids', # e:id
    'relation2ids': '/rel2ids-transd' # r:id 
}
