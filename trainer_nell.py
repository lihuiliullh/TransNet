...
from models_nell import Hire
#from models_naacl import Hire
...
class Trainer:
    ...
    def load_embed(self):
        ...
        if self.embed_model in ['DistMult', 'TransE', 'ComplEx', 'RESCAL']:
            ...
            for key in rel2id.keys():
                if key not in ['','OOV']: # Lihui: delete "" and oov from embedding 
                    symbol_id[key] = i
                    i += 1
    ...
    def build_connection(self, max_=100):
        # Lihui: matrix initialized as pad_id
        # Lihui: only store at most max_ neighbors 
        ...
        for ent, id_ in self.ent2id.items():
            ...
            for idx, _ in enumerate(neighbors):
                self.connections[id_, idx, 0] = _[0] # Lihui: this is relation id
                self.connections[id_, idx, 1] = _[1] # Lihui: this is tail entity id
    ...
    #
    # self.few = 2
    # self.ent2id = {'e1': 0, 'e2': 1, 'e3': 2, 'e4': 3}
    # Assume task contains:
    #
    # task = [
    #     # Support triples
    #     [[('e1', 'rel1', 'e2'), ('e3', 'rel1', 'e4')],  # Relation 1
    #      [('e2', 'rel2', 'e1'), ('e4', 'rel2', 'e3')]], # Relation 2
    #
    #     # Support negative triples
    #     [[('e1', 'rel1', 'e3'), ('e3', 'rel1', 'e1')],  # Relation 1
    #      [('e2', 'rel2', 'e4'), ('e4', 'rel2', 'e2')]], # Relation 2
    #
    #     # Query triples
    #     [[('e1', 'rel1', 'e4'), ('e3', 'rel1', 'e2')],  # Relation 1
    #      [('e2', 'rel2', 'e3'), ('e4', 'rel2', 'e1')]], # Relation 2
    #
    #     # Negative query triples
    #     [[('e1', 'rel1', 'e1'), ('e3', 'rel1', 'e3')],  # Relation 1
    #      [('e2', 'rel2', 'e2'), ('e4', 'rel2', 'e4')]]  # Relation 2
    # ]
    # Breakdown with np.transpose
    # Support Triples for Relation 1 and Relation 2:
    #
    # triplet_list = [
    #     [('e1', 'rel1', 'e2'), ('e3', 'rel1', 'e4')],  # Relation 1
    #     [('e2', 'rel2', 'e1'), ('e4', 'rel2', 'e3')]   # Relation 2
    ...
    # python
    # Copy code
    # triplet_left = [0, 2, 1, 3]  # [e1, e3, e2, e4]
    # triplet_right = [1, 3, 0, 2]  # [e2, e4, e1, e3]
    # Reshape into few-shot examples:
    ...
    # # [[1, 3], [0, 2]]
    # Transpose these chunks:

    # python
    # Copy code
    # triplet_meta_left = np.transpose(np.array([[0, 2], [1, 3]]))
    # # [[0, 1], [2, 3]]

    # triplet_meta_right = np.transpose(np.array([[1, 3], [0, 2]]))
    # # [[1, 0], [3, 2]]
    # Calling get_meta(): For each relation:

    # python
    # Copy code
    # self.get_meta([0, 1], [1, 0])  # For Relation 1
    # self.get_meta([2, 3], [3, 2])  # For Relation 2
    # Support Negative Triples: Extract entity IDs and process similarly:
    ...
    # python
    # Copy code
    # triplet_left = [0, 2, 1, 3]  # Same IDs as support triples
    # triplet_right = [2, 0, 4, 2]  # New tail IDs for negative samples
    ...
    # triplet_meta_right = np.transpose(np.array([[2, 0], [4, 2]]))
    # # [[2, 4], [0, 2]]
    # Query Triples: Extract IDs:

    # python
    # Copy code
    # triplet_left = [0, 2, 1, 3]  # Query heads
    # triplet_right = [3, 1, 2, 0]  # Query tails
    # Reshape and transpose:
    # python
    # Copy code
    # triplet_meta_left = np.transpose(np.array([[0, 2], [1, 3]]))
    ...
    # triplet_meta_right = np.transpose(np.array([[3, 1], [2, 0]]))
    # # [[3, 2], [1, 0]]
    # Negative Query Triples: Extract IDs and process similarly:

    # python
    # Copy code
    # triplet_left = [0, 2, 1, 3]  # Negative query heads
    # triplet_right = [0, 2, 1, 3]  # Negative query tails
    # Reshape and transpose:
    # python
    # Copy code
    # triplet_meta_left = np.transpose(np.array([[0, 2], [1, 3]]))
    ...
    # triplet_meta_right = np.transpose(np.array([[0, 2], [1, 3]]))
    # # [[0, 1], [2, 3]]
    # Final Example Output
    # Incorporating np.transpose, the output for each set in task_meta could look like this:

    # python
    # Copy code
    # task_meta = [
    #     # Support triples metadata for both relations
    #     [
    #         ([0, 1], [1, 0]),  # Relation 1
    #         ([2, 3], [3, 2])   # Relation 2
    #     ],

    #     # Support negative triples metadata for both relations
    #     [
    #         ([0, 1], [2, 4]),  # Relation 1 negative
    #         ([2, 3], [0, 2])   # Relation 2 negative
    #     ],

    #     # Query triples metadata for both relations
    #     [
    #         ([0, 1], [3, 2]),  # Relation 1 query
    #         ([2, 3], [1, 0])   # Relation 2 query
    #     ],

    #     # Negative query triples metadata for both relations
    #     [
    #         ([0, 1], [0, 1]),  # Relation 1 query negative
    #         ([2, 3], [2, 3])   # Relation 2 query negative
    #     ]
    # ]
    def get_task_meta(self, task, iseval): # Lihui: task could be [support, support_negative, query, negative] or [support]
        task_meta = []
        for triplet_list in task: # Lihui: triplet_list is a list of list, the most inside list is k few-shot examples of r. The outlist is for different relations
            triplet_left = [self.ent2id[few[0]] for batch in triplet_list for few in batch] # Lihui: should rename batch to support or query
            triplet_right = [self.ent2id[few[2]] for batch in triplet_list for few in batch]
            if not iseval:
                triplet_meta_left = [triplet_left[i:i+self.few] for i in range(0, len(triplet_left), self.few)]
                triplet_meta_left = np.transpose(np.array(triplet_meta_left)).tolist() # Lihui: few_shot_number x bath_size(different relation number)
                triplet_meta_right = [triplet_right[i:i+self.few] for i in range(0, len(triplet_right), self.few)]
        ...
        return task_meta # [[(left: [1024, 50, 2], degree, right, degree)], [support_negative], [query], xxx] when few_shot_num=1, batch_size=1024
        # if few_shot_num = 2, task_meta will be [[(left: [1024, 50, 2], degree, right, degree), (left: [1024, 50, 2], degree, right, degree)], [support_negative], [query], xxx]
    
    def get_meta(self, left, right): # Lihui: this is for context contrastive learning 
        left_connections = Variable(torch.LongTensor(np.stack([self.connections[_, :, :] for _ in left], axis=0))).to(self.device) # Lihui: get all neighbors of left
        left_degrees = Variable(torch.LongTensor([self.e1_degrees[_] for _ in left])).to(self.device)
        right_connections = Variable(torch.LongTensor(np.stack([self.connections[_, :, :] for _ in right], axis=0))).to(self.device) # Lihui: get all neighbors of right
    ...
    def do_one_step(self, task, iseval=False, curr_rel='', istest=False):
        ...
        # Lihui: meta here meaning neighbor context
        support_meta = self.get_task_meta([task[0]], iseval)[0] # Lihui: [(left: [1024, 50, 2], degree, right, degree), (left: [1024, 50, 2], degree, right, degree), (left: [1024, 50, 2], degree, right, degree)] when few_shot=3
        support_negative_meta = []
        for i in range(self.max_nn_meta):
            support_negative_meta.append(self.get_negative_meta(support_meta)) # Lihui: generate negative neighbor
        if not iseval:
            ...
            y = torch.Tensor([1] * p_score.shape[0]).unsqueeze(dim=1).to(self.device) # LIhui: I revise here
            ...
            y = torch.Tensor([1] * p_score.shape[0]).unsqueeze(dim=1).to(self.device)
            loss_margin = self.hire.loss_func(p_score, n_score, y)
            loss_info = 0
    ...
    def train(self):
        ...
        for e in range(self.epoch):
            train_task, curr_rel = self.train_data_loader.next_batch() # Lihui: train_task is [support, support_negative, query, negative]
